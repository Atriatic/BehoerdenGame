...
export default function CityView({ resources }: Props) {
  // Low states
  const budgetLow = resources.budget < 35;
  const budgetCritical = resources.budget < 15;
  const unhappy = resources.zufriedenheit < 35;
  const veryUnhappy = resources.zufriedenheit < 15;
  const understaffed = resources.personal < 35;
  const veryUnderstaffed = resources.personal < 15;
  const chaotic = resources.effizienz < 35;

  // High states (v1.5 Dual-Failure Spiegel)
  const budgetHigh = resources.budget > 78;
  const happyHigh = resources.zufriedenheit > 78;
  const staffedHigh = resources.personal > 78;
  const efficientHigh = resources.effizienz > 78;

  const skyColor = budgetCritical
    ? '#4B5563'
    : budgetLow
    ? '#7A8EA0'
    : budgetHigh
    ? '#FEF3C7'
    : '#87CEEB';
  const buildingTone = budgetCritical
    ? '#9CA3AF'
    : budgetLow
    ? '#B0B8C4'
    : budgetHigh
    ? '#F9D77E'
    : '#E8D5B7';
  const rathausColor = budgetLow ? '#8B9DB0' : budgetHigh ? '#C4956A' : '#C4956A';
      ...
      >
        <defs>
          <linearGradient id="skyGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop
              offset="100%"
              stopColor={budgetLow ? '#CBD5E1' : budgetHigh ? '#FDE68A' : '#E0F0FF'}
            />
          </linearGradient>
          <linearGradient id="groundGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={chaotic ? '#78350F' : efficientHigh ? '#166534' : '#4A7C59'} />
        ...
        {/* Rechnungshof-Dienstwagen (Budget hoch) */}
        {budgetHigh && (
          <g>
            <rect x="310" y="225" width="50" height="14" fill="#1F2937" rx="2" />
            <rect x="314" y="218" width="42" height="10" fill="#374151" rx="2" />
            <circle cx="322" cy="240" r="5" fill="#6B7280" />
            <circle cx="352" cy="240" r="5" fill="#6B7280" />
            <rect x="320" y="220" width="12" height="6" fill="#93C5FD" opacity="0.7" rx="1" />
            <rect x="335" y="220" width="12" height="6" fill="#93C5FD" opacity="0.7" rx="1" />
            {/* Kennzeichen */}
            <rect x="327" y="228" width="18" height="6" fill="#DBEAFE" rx="1" />
            <text x="328" y="233" fontSize="4" fill="#1E3A8A" fontWeight="bold">RH-01</text>
          </g>
        )}
        ...
        <rect x="48" y="192" width="14" height="14" fill={veryUnderstaffed ? '#374151' : '#FDE68A'} rx="1" />
        ...
        {/* Goldener Glanz bei Budget hoch */}
        {budgetHigh && (
          <rect x="155" y="130" width="90" height="95" fill="#FEF3C7" opacity="0.3" />
        )}
        ...
        <rect x="166" y="186" width="18" height="16" fill={veryUnderstaffed ? '#374151' : '#FDE68A'} rx="1" />
        <rect x="218" y="186" width="18" height="16" fill={veryUnderstaffed ? '#374151' : '#FDE68A'} rx="1" />
        {/* Haupttür */}
        ...
        <rect x="270" y="180" width="14" height="14" fill={veryUnderstaffed ? '#374151' : '#FDE68A'} rx="1" />
        <rect x="292" y="180" width="14" height="14" fill={veryUnderstaffed ? '#374151' : '#FDE68A'} rx="1" />
        <rect x="270" y="202" width="14" height="14" fill={veryUnderstaffed ? '#374151' : '#FDE68A'} rx="1" />
        ...
        <ellipse cx="139" cy="200" rx="14" ry="18" fill={efficientHigh ? '#166534' : chaotic ? '#6B7280' : '#2D6A4F'} />
        <rect x="310" y="208" width="6" height="22" fill="#92400E" />
        <ellipse cx="313" cy="203" rx="13" ry="16" fill={efficientHigh ? '#166534' : chaotic ? '#6B7280' : '#2D6A4F'} />

        {/* ─── Aktenstapel (Effizienz niedrig) ─── */}
        {chaotic && (
          <g>
            ...
            <rect x="163" y="98"  width="15" height="2" fill="#FEF3C7" opacity="0.4" />
          </g>
        )}

        {/* ─── Viele Beamte sichtbar (Personal hoch) ─── */}
        {staffedHigh && (
          <g>
            {[160, 175, 190, 210, 225].map((x, i) => (
              <g key={i}>
                <circle cx={x} cy={226} r={4} fill="#FBBF24" />
                <rect x={x - 3} y={230} width={6} height={7} fill={['#3B82F6','#10B981','#8B5CF6','#F59E0B','#EF4444'][i]} />
              </g>
            ))}
            {/* Schild: viele Mitarbeiter = verdächtig voll */}
            <rect x="150" y="207" width="90" height="12" fill="#FEF3C7" rx="2" opacity="0.9" />
            <text x="195" y="216" fontSize="5" fill="#92400E" textAnchor="middle" fontWeight="bold">Personalrat tagt permanent</text>
          </g>
        )}

        {/* ─── Bürger normal ─── */}
        {!veryUnhappy && !staffedHigh && (
          <g>
        ...
        {/* Protest-Bürger (Zufriedenheit niedrig) */}
        {unhappy && !staffedHigh && (
          <g>
        ...
        {/* Fackelträger (sehr unzufrieden) */}
        {veryUnhappy && (
          <g>
            {[145, 165, 185].map((x, i) => (
              <g key={i}>
                <circle cx={x} cy={225} r={5} fill="#FBBF24" />
                <rect x={x - 3} y={230} width={6} height={8} fill="#EF4444" />
                <rect x={x + 2} y={212} width={2} height={14} fill="#78350F" />
                <ellipse cx={x + 3} cy={210} rx={4} ry={5} fill="#F97316" opacity={0.9} />
                <ellipse cx={x + 3} cy={208} rx={2} ry={3} fill="#FCD34D" opacity={0.8} />
              </g>
            ))}
          </g>
        )}

        {/* Zufriedenheit hoch: Blumenschilder, zu freundlich */}
        {happyHigh && (
          <g>
            <rect x="90" y="210" width="55" height="10" fill="#D1FAE5" rx="2" opacity="0.9" />
            <text x="117" y="218" fontSize="5" fill="#065F46" textAnchor="middle" fontWeight="bold">♥ Wir lieben unser Amt! ♥</text>
          </g>
        )}

        {/* Normaler Bürger rechts */}
        {!staffedHigh && (
          <g>
        ...
        {/* Bröckeleffekte bei Budget-Krise */}
        {budgetCritical && (
          <g opacity="0.6">
        ...
        {/* Dunkler Overlay bei kritischen Zuständen */}
        {budgetCritical && (
          <rect x="0" y="0" width="400" height="300" fill="#1F2937" opacity="0.28" />
        )}
      </svg>

      {/* Verlauf zum Spiel-Inhalt */}
      <div className="absolute inset-0 bg-gradient-to-t from-petrol/55 via-transparent to-transparent pointer-events-none" />
    </div>
  );
}
